...
def evaluate_lda_vote_single_fold(fold_idx):
    """
    使用三个序列的特征，分别训练 LDA，然后用软投票评估
    """
    print(f"\n{'='*20} Evaluating Fold {fold_idx} {'='*20}")
    print(f"Mode: Late Fusion (Soft Voting) | Model: LDA")
    ...
    for seq_id in range(1, len(ALL_SEQUENCES) + 1):
        ...
        if not cache_file.exists():
            ...
            print(f"Please run 'python baseline_scripts/lda.py --seq {seq_id}' first to extract features.")
            return None
        ...
        prob_dist = lda.predict_proba(X_test_pca.astype(np.float64))
        seq_probs.append(prob_dist)

    print(f"  -> Successfully fitted LDA for 3 sequences and extracted probabilities. (Took {time.time()-t0:.2f}s)")
...
# ================== 主流程 ==================
def main(args):
    print(f"\n>>> Starting K-Fold Evaluation for: LDA Late Fusion Soft Voting <<<")
    ...
    for k in folds_to_run:
        res = evaluate_lda_vote_single_fold(k)
        if res:
