...
外部验证使用`external_eval.py`，该流程对外部数据即时执行与原始数据一致的预处理步骤：重采样、Z-score归一化、中心裁剪/填充，然后使用当前最佳异构模型进行三序列软投票。脚本会额外输出诊断CSV，包含每个病例、每个fold、每个序列模型的三类概率，以及fold内投票和最终跨fold平均后的预测结果；默认保存到`version1/inference_outputs/`，也可以通过`--csv_path`指定。
